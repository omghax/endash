...
(function(){
	...
	function makeAttributesString(tag, profile) {
		...
		var attrs = '',
			...
			cursor = profile.place_cursor ? Endash.ZenCoding.getCaretPlaceholder() : '',
			attr_name;
	...
	function processSnippet(item, profile, level) {
		...
		item.start = item.start.replace('%s', Endash.ZenCoding.padString(start, padding));
		item.end = item.end.replace('%s', Endash.ZenCoding.padString(end, padding));
		...
		item.start = Endash.ZenCoding.replaceVariables(item.start, cb);
		item.end = Endash.ZenCoding.replaceVariables(item.end, cb);
	...
	function process(tree, profile, level) {
		level = level || 0;
		if (level == 0) {
			tree = Endash.ZenCoding.runFilters(tree, profile, '_format');
			tabstops = 0;
		}
		...
		for (var i = 0, il = tree.children.length; i < il; i++) {
			...
			// replace counters
			var counter = Endash.ZenCoding.getCounterForNode(item);
			item.start = Endash.ZenCoding.unescapeText(Endash.ZenCoding.replaceCounter(item.start, counter));
			item.end = Endash.ZenCoding.unescapeText(Endash.ZenCoding.replaceCounter(item.end, counter));
			item.content = Endash.ZenCoding.unescapeText(Endash.ZenCoding.replaceCounter(item.content, counter));
			
			tabstops += Endash.ZenCoding.upgradeTabstops(item, tabstops) + 1;
	...
	Endash.ZenCoding.registerFilter('html', process);
})();