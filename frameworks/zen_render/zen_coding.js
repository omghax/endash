...
Endash.ZenCoding = (function(){
	 ...
	 */
	function getNewline() {
		return Endash.ZenCoding.getNewline();
	}
	...
	Tag.prototype = {
		...
		setPasteContent: function(val) {
			this._paste_content = Endash.ZenCoding.escapeText(val);
		},
	...
	function getAbbreviation(type, abbr) {
		return Endash.ZenResources.getAbbreviation(type, abbr);
	}
	...
	function getSnippet(type, snippet_name) {
		return Endash.ZenResources.getSnippet(type, snippet_name);
	}
	...
	function getVariable(name) {
		return Endash.ZenResources.getVariable(name);
	}
	...
	ZenNode.prototype = {
		...
		isUnary: function() {
			...
			return (this.source._abbr && this.source._abbr.value.is_empty) 
				|| Endash.ZenResources.isItemInCollection(this.source.syntax, 'empty', this.name);
		},
		...
		isInline: function() {
			return this.type == 'text' 
				|| Endash.ZenResources.isItemInCollection(this.source.syntax, 'inline_level', this.name);
		},
	...
	function replaceExpandos(node, type) {
		for (var i = 0, il = node.children.length; i < il; i++) {
			...
			if (!n.isEmpty() && !n.isTextNode() && n.name.indexOf('+') != -1) {
				...
				if (a)
					node.children[i] = Endash.ZenParser.parse(a.value);
			}
	...
	function preprocessParsedTree(tree, type) {
		...
		return Endash.ZenParser.optimizeTree(tree);
	}
...
//			}
		...
		parseIntoTree: function(abbr, type) {
			...
			try {
				var abbr_tree = Endash.ZenParser.parse(abbr),
		...
		applyFilters: function(tree, syntax, profile, additional_filters) {
			...
			if (!_filters)
				_filters = Endash.ZenResources.getSubset(syntax, 'filters') || basic_filters;
		...
		setVariable: function(name, value){
			var voc = Endash.ZenResources.getVocabulary('user') || {};
			if (!('varaibles' in voc))
				voc.variables = {};
				
			voc.variables[name] = value;
			Endash.ZenResources.setVocabulary(voc, 'user');
		},
